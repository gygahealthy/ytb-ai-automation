...
import { ChannelCompetitor, CreateCompetitorInput } from '../../../main/modules/channel-management/youtube.types';
...
const CompetitorTracking: React.FC<Props> = ({ channelId, competitors, onUpdate }) => {
  ...
  const handleAddCompetitor = async () => {
    if (!competitorName.trim() || !competitorUrl.trim() || !competitorChannelId.trim()) return;
      ...
      <div className="bg-gradient-to-r from-orange-50 to-red-50 dark:from-gray-800 dark:to-gray-850 px-6 py-4 border-b border-gray-200 dark:border-gray-800 flex-shrink-0">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-bold text-gray-900 dark:text-white">Competitor Tracking</h2>
              <p className="text-xs text-gray-500 dark:text-gray-400">{competitors.length} competitors tracked</p>
            </div>
          </div>
          <button
      ...
      <div className="flex-1 overflow-y-auto p-6">
          ...
          <div className="mb-4 bg-gray-50 dark:bg-gray-800 rounded-xl p-4 border border-gray-200 dark:border-gray-700">
            ...
            <div className="flex gap-2">
              <button
                ...
                disabled={saving || !competitorName.trim() || !competitorUrl.trim()}
        ...
        {competitors.length === 0 ? (
          <div className="text-center py-8">
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400 font-medium mb-1">No competitors tracked yet</p>
            <p className="text-xs text-gray-400 dark:text-gray-500">Click the + button to add your first competitor</p>
          </div>
        ) : (
          <div className="space-y-3">
            {competitors.map((competitor) => (
              <div key={competitor.id} className="bg-gray-50 dark:bg-gray-800 rounded-xl p-4 hover:bg-gray-100 dark:hover:bg-gray-750 transition-all duration-200 border border-gray-200 dark:border-gray-700 group">
                    ...
                    <div className="flex items-center gap-4 text-xs">
                      {competitor.subscriberCount && (
                        <div className="flex items-center gap-1.5 text-gray-600 dark:text-gray-400">
                          ...
                          <span className="text-gray-400 dark:text-gray-500">subs</span>
                        </div>
                      )}
                      {competitor.avgViews && (
                        <div className="flex items-center gap-1.5 text-gray-600 dark:text-gray-400">
                      ...
                      {competitor.uploadFrequency && (
                        <div className="flex items-center gap-1.5 text-gray-600 dark:text-gray-400">
                          <span>📅</span>
                          <span className="font-medium">{competitor.uploadFrequency}</span>
                        </div>
                      )}
                    </div>
                    {competitor.notes && (
                      <p className="mt-2.5 text-xs text-gray-500 dark:text-gray-400 leading-relaxed">{competitor.notes}</p>
                    )}
                  </div>
                  <button
