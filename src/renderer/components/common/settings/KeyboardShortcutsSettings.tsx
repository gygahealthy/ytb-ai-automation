import { Code, History, Keyboard, Pin, RefreshCw, Terminal, User, ArrowLeft, ArrowRight, Menu, LayoutGrid } from "lucide-react";
...
export default function KeyboardShortcutsSettings() {
  ...
  const pageSpecificShortcuts = shortcuts.filter(
    (s) => s.id === "cycle-video-creation-mode" || s.id === "toggle-video-properties"
  );
  const appWideShortcuts = shortcuts.filter((s) => !pageSpecificShortcuts.find((ps) => ps.id === s.id));

  const renderShortcutItem = (shortcut: any) => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-3">
        ...
        {appWideShortcuts.map(renderShortcutItem)}
      </div>

      {/* Page-specific Shortcuts */}
      {pageSpecificShortcuts.length > 0 && (
        <div className="space-y-3">
          <h4 className="text-sm font-semibold text-gray-700 dark:text-gray-300 flex items-center gap-2">
            <LayoutGrid className="w-4 h-4" />
            Page-specific Shortcuts
          </h4>
          <p className="text-xs text-gray-500 dark:text-gray-400 -mt-2">These shortcuts only work on specific pages</p>
          {pageSpecificShortcuts.map(renderShortcutItem)}
        </div>
      )}
