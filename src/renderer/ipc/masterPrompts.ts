...
const getByKind = (kind: string) => {
  ...
  if (hasInvoke()) return invoke('master-prompts:getByKind', kind);
  return Promise.resolve({ success: false, error: 'ipc-not-available' });
};
...
export default { getAll, getById, getByProvider, getByKind, createPrompt, updatePrompt, deletePrompt, archivePrompt };
