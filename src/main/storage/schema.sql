...
-- Cookies table for storing Gemini API authentication cookies
...
-- Cookie rotation monitoring table
-- Tracks the health and status of cookie rotation workers for each profile
CREATE TABLE IF NOT EXISTS cookie_rotation_monitors (
  ...
  last_error_at TEXT,
  requires_headless_refresh INTEGER DEFAULT 0, -- 0 = false, 1 = true (triggers headless fallback)
...
CREATE INDEX IF NOT EXISTS idx_veo3_video_generations_profile_id ON veo3_video_generations(profile_id);
CREATE INDEX IF NOT EXISTS idx_veo3_video_generations_status ON veo3_video_generations(status);
