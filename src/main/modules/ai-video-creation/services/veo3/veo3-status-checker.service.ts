...
export class VEO3StatusCheckerService {
  ...
  async checkGenerationStatus(generationId: string): Promise<ApiResponse<any>> {
    try {
      ...
      const cookieResult = await cookieService.getCookiesByProfile(
      ...
      const statusResult = await veo3ApiClient.checkVideoStatus(
        bearerToken,
      ...
      const mediaStatus =
        statusData?.mediaStatus || statusData?.status || "UNKNOWN";
      ...
      if (
        ...
        mediaStatus === "MEDIA_GENERATION_STATUS_COMPLETED"
      ) {
        const extracted = extractVideoMetadata(rawData);
        await videoGenerationRepository.updateStatus(
          generationId,
          "completed",
          {
        ...
        mediaStatus === "MEDIA_GENERATION_STATUS_FAILED" ||
        mediaStatus === "FAILED"
      ) {
        const errorMessage =
          statusData?.error || statusData?.errorMessage || "Unknown error";
        ...
        logger.error(
          `❌ Video generation failed: ${generationId} - ${errorMessage}`
        );
        return {
          success: true,
          data: {
  ...
  async refreshVideoStatus(
    ...
    try {
      logger.info(
        `Manually refreshing video status for generation: ${generationId}`
      );
      ...
      if (
        ...
        actualStatus === "MEDIA_GENERATION_STATUS_FAILED" ||
        actualStatus === "FAILED" ||
        status === "failed"
      ) {
        await videoGenerationRepository.updateStatus(generationId, "failed", {
        ...
        logger.info(
          `⏳ Video status refreshed - still processing: ${generationId}`
        );
        return {
          success: true,
          data: {
   ...
   */
  async listGenerationsByProfile(
    profileId: string,
    limit: number = 50,
    offset: number = 0
  ): Promise<ApiResponse<any[]>> {
    try {
      const generations = await videoGenerationRepository.getByProfile(
        profileId,
        limit,
        offset
      );
