...
import { promptHistoryRepository } from '../repository/prompt-history.repository';
import { CreatePromptHistoryInput } from '../repository/prompt-history.repository';
...
class PromptHistoryService {
	async getByPromptId(req: any): Promise<ApiResponse<any[]>> {
		try {
			...
			const data = await repository.getByPromptId(Number(promptId), Number(limit));
	...
	async create(req: any): Promise<ApiResponse<any>> {
		try {
			...
			if (String(err) === 'Error: duplicate' || (err && err.message === 'duplicate')) {
