...
export function cookieMatches(
  ...
  return cookies.some(
    (c: any) => typeof c?.name === "string" && c.name === query
  );
}
 ...
 */
export function hasCookieChanged(
  previousCookies: SimpleCookie[] | any[],
  currentCookies: SimpleCookie[] | any[]
): boolean {
  ...
  const previousSet = new Set<string>(
    previousCookies.map((c: any) => `${c.name}=${c.value}`)
  );
  const currentSet = new Set<string>(
    currentCookies.map((c: any) => `${c.name}=${c.value}`)
  );
